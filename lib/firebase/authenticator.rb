...
module Firebase
  ...
  class Authenticator
    ...
    def self.verify_token(token)
      ...
      raise StandardError, "Invalid token alg header #{alg}" unless alg == JWT_ALGORITHM

      public_key = get_public_key(kid)

      decoded_token = decode_token(token, public_key, Rails.application.credentials.firebase_project_id)

      valid_token = valid_token?(decoded_token['exp'])

      raise StandardError, 'Token expired' unless valid_token
    ...
    def self.get_public_key(kid)
      ...
      raise StandardError, '`invalid kid header' unless public_keys.include?(kid)
